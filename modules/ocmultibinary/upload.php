...
if ( strpos( $contentType, 'multipart' ) !== false )
{
    if ( isset( $_FILES['file']['tmp_name'] ) && is_uploaded_file( $_FILES['file']['tmp_name'] ) )
    {
        ...
        if ( $out )
        {
            ...
            {
                eZDebug::writeError( 'Failed to move uploaded file.', 'ocmultibinary/upload' );
...
else
{
    ...
    if ( $out )
    {
        ...
        {
            eZDebug::writeError( 'Failed to open input stream.', 'ocmultibinary/upload' );
            die( '{"jsonrpc" : "2.0", "error" : {"code": 101, "message": "Failed to open input stream."}, "id" : "id"}' );
        }
        fclose( $out );
        $oldumask = umask( 0 );
    ...
    {
        eZDebug::writeError( 'Failed to open output stream.', 'ocmultibinary/upload' );
        die( '{"jsonrpc" : "2.0", "error" : {"code": 102, "message": "Failed to open output stream."}, "id" : "id"}' );
    }
}
